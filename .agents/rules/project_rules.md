...
2. **計画優先 (Planning Mode)**: 大きな変更や新しいファイルの作成を行う前に、必ず `implementation_plan.md` を作成または更新し、ユーザーの承認を得ること。
   - **デグレ・リスク評価の必須化**: どのような案件・機能改修であっても、`implementation_plan.md` 内には必ず【リスク評価とデグレ防止設計 (Risk Analysis & Safeguards)】のセクションを設け、潜在的リスク（APIクォータ・既存UI崩れ・DBスキーマ/キャッシュ影響・パフォーマンス遅延など）と防護策を明確に記述すること。
