...
* **💡 前日比 (DoD) ＆ 前週比 (WoW) インタラクティブバッジツールチップ (#62)**:
  ...
  * 実日付 (`recorded_at`) を基準とした最近傍検索アルゴリズムと `useMemo` 事前計算により 60fps スムース描画を確保。
* **🎬 プレミア公開 (Premiere) 動画の分離除外 ＆ LIVE 判定精度100%化 (#59)**:
  * 通常動画の初公開指定（プレミア公開）を自動識別し、誤った LIVE 配信割合の算出（@studytimer 等の誤カウント）を完全解決。
* **🛡️ 新規登録直後の画面点滅・無限ループ完全根絶 (#60)**:
  * 時系列データ 0件時のフェッチ制御 `isHistoryLoaded` および登録時初期データ保存により、高速な連続再描画を100%防止。
* **🛡️ 起動時全自動レスキュー ＆ Actions日次フェッチ網羅 (#52, #58)**:
  * バックエンド起動時の自動補テンインポートエラー修復と `fetch_stats.py` 全件フェッチ対応。
