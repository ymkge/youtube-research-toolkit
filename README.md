...
### 📊 ダッシュボード (メイン画面)
* **🔄 親Channelサマリー数値 ＆ 実態履歴の完全自動同調・修復機能 (#84)**:
  * **完全同期保証**: カード中央のサマリー数値（登録者数・総再生数・動画数）と、グラフ内の最新履歴数値が常に 100% 完全一致することを保証。
  * **🛡️ セルフヒーリング (動的自動修復)**: 一覧取得 API (`GET /api/channels`) レスポンス生成時、万が一親 Channel カラムが過去のデータであっても、最新履歴データで自動補正し DB 内も自動修復。
...
### 📈 成長率比較分析画面 (#28, #35, #42, #48, #61, #82)
* **📌 ピン留め ＆ 🔥 急成長 ＆ 📉 衰退 シグナル一括選択フィルター (#82)**:
  * 左サイドバーにメインダッシュボードと 100% 統一された判定基準に基づく **`📌 ピン留め`**, **`🔥 急成長`**, **`📉 衰退`** のワンタップ一括選択ボタンを追加。注目・急伸び・減退中のチャンネルを一瞬で選択してタイムライン比較可能。
